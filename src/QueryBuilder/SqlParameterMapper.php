...
class SqlParameterMapper
{
    public function map(MetricParameter $parameter): string
    {
        ...
        return match ($parameter) {
            ...
            MetricParameter::ENTRY_PAGE => "(SELECT pathname FROM events e2 WHERE e2.session_id = e2.session_id ORDER BY occurred_on ASC LIMIT 1)",
            MetricParameter::EXIT_PAGE => "(SELECT pathname FROM events e2 WHERE e2.session_id = e2.session_id ORDER BY occurred_on DESC LIMIT 1)",
