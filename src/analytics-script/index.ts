...
(async function () {
    ...
    if (window.__LUKALTA_OPTOUT__ || localStorage.getItem("disable-luka-lta") !== null) {
    ...
    function setupEventListeners() {
        ...
        document.addEventListener("click", function (e) {
            ...
            while (target && target !== document.documentElement) {
                if (target.hasAttribute("data-lukalta-event")) {
                    const eventName = target.getAttribute("data-lukalta-event");
                    if (eventName) {
                        const properties: Record<string, string> = {};
                        for (const attr of target.attributes) {
                            if (attr.name.startsWith("data-lukalta-prop-")) {
                                const propName = attr.name.replace("data-lukalta-prop-", "");
